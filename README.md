...
.tools/python-env/bin/python -m tools.capture_serial --port /dev/cu.usbmodem4 \
  --output .local/runs/diagnostic-001 --seconds 300 --reset \
...
Startup includes sequential 60-second camera, audio and motion baselines, plus short live and synthetic audio fixtures. Allow at least five minutes for a full capture; this is a diagnostic build, not the final startup experience. Assess each timing capture with `python3 -m tools.camera_baseline_evidence METADATA.json`, `tools.audio_baseline_evidence`, or `tools.imu_baseline_evidence`. Camera checks count missing application buffers, and motion checks retain every latest-register polling attempt. Passing isolated stages does not establish combined-load behavior. No microSD is currently installed, so `sd_roundtrip` remains inconclusive.
