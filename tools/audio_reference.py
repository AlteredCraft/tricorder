...
def pcm_fixtures():
    ...
    return {name:[max(-32768,min(32767,round(x*32768))) for x in samples]
            for name,samples in fixtures.items()}


def write_header(path):
    lines=['// Generated by tools.audio_reference --header; synthetic PCM fixtures.',
           '#pragma once', '#include <cstdint>', '#include <cstddef>',
           'struct DspFixture { const char* name; const int16_t* pcm; size_t frames; };']
    fixtures=pcm_fixtures()
    for name,pcm in fixtures.items():
        lines.append(f'static constexpr int16_t fixture_{name}[] = {{')
        lines.extend('    '+','.join(map(str,pcm[i:i+16]))+',' for i in range(0,len(pcm),16))
        lines.append('};')
    lines.append('static constexpr DspFixture dsp_fixtures[] = {')
    lines.extend(f'    {{"{name}",fixture_{name},2048}},' for name in fixtures)
    lines.append('};')
    path.write_text('\n'.join(lines)+'\n')
...
def main():
    ...
    mode.add_argument('--header',type=Path)
