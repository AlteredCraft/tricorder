...
**Implementation resumed, 2026-09-21:** G-0002's host protocol/mock checkpoint is built; firmware integration and physical trials are next. The user selected a speaker playing a steady sound for the A/B fixture. Previous owned-preview/JPEG evidence remains intact; this increment changes no firmware and opens no USB port. See [HANDOFF.md](HANDOFF.md#resume-here).
...
- **Handheld interaction:** first help freeze the selected steady-speaker distance fixture (source level, room/background and placement), then run G-0002's prepared mock/live loops with responsiveness/usefulness feedback. Later supply G-0001's combined-run touch events, supported touch/motion wake trials and full guided-investigation acceptance runs. Early slice evidence counts toward those only if it meets their original checks.
