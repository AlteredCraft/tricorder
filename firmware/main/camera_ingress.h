...
#include <cstdint>
#include <cstddef>
struct CameraIngressSnapshot {uint64_t requests=0,missing_buffers=0,finished=0,bytes=0,untracked_buffers=0,reused_completions=0;size_t expected_bytes=0;};
...
// Pinned CSI callback registration supplies its esp_video owner as user data.
bool camera_owns_video(const void* video);
