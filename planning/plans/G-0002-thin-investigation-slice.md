...
**Status.** In progress. SD setup, verified capture archives/replay and bounded TCP completion are implemented. A fresh physical mock A/B loop completes with independent verification (B/A −3.89 dB); historical failures remain retained and their exact transport cause is unproven. OpenRouter `openai/gpt-5.6-sol` passes two live calls over saved summaries. Speech, three operator-rated live loops, full fixture/timing evidence and original reliability acceptance remain open.
...
**Progress — 2026-09-22, SD-assisted testing.** Wi-Fi and the mock endpoint now survive resets. Both real A/B recordings were recovered and independently verified after each of two failed physical attempts. A tested memory correction addresses the first attempt's radio DMA allocation assertion; the retry stayed running but failed during B upload. Three labeled SD replays, including a slowed transfer, complete the original evidence-linked protocol with matching raw bytes and metadata. Replay avoids new acquisition and is not another physical acceptance run. 180 host tests and the P4 build pass. Exact evidence, remaining transport uncertainty and the repeatable replay procedure are linked from [HANDOFF.md](../../HANDOFF.md#current-checkpoint). All trial processes are stopped. Live-provider/speech/usefulness work and the original G-0001 gates remain open; no goal is marked Met.

**Progress — 2026-09-22, continuation:** User authorized all four stages through original G-0001 reliability acceptance and selected OpenAI. Transport short-write regression and a bounded completion candidate are underway. Current Wi-Fi and local API credential paths are requested; prior failures and thresholds remain unchanged.


**Progress — 2026-09-22, commit checkpoint.** Fresh physical session `ab-37d8b855eaf9b6a50662cc17bcd1f9ad` passes raw/ingress/transcript checks; operator confirms Comparison. B/A is −3.8894 digital dB. Earlier interrupted and failed sessions remain incomplete. 192 host tests and P4 build/flash pass. ADR-0011/0012 adopt bounded TCP completion and live prose over host-owned measurements. Trial processes are closed; speech and all original acceptance gates remain open. See the current HANDOFF and slice observations for evidence paths.
