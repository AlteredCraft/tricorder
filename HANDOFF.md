...
**Pending operator question:** third and final near-connector recording, expected `TAKE audio-3`; requested same position, covered farther hole, phrase and distance. Verify completion and ask for the three farther-hole takes one at a time. User missed audio-1's visible ID but reports slot 0 best quality, 2 good, 1/3 no audio; log uniquely matches that request. User explicitly saw `TAKE audio-2` and reports completion. Both driver epochs pass. Earlier `speech-replay-1` reported three takes per position but logged only two operator captures: preserve the mismatch and do not infer mapping. Current near-connector slot-0/2 RMS ratios are -2.76 and -1.81 dB, insufficient alone for mapping.
