...
Current firmware automatically runs a 60-second camera baseline, three live raw/speech captures, synthetic FFT/speech fixtures and a 60-second audio baseline. Manual record/restart/audio buttons remain available. **78 host tests and the P4 build pass.** Artifacts: `.local/runs/20260920-baseline/camera-baseline-build-2-artifacts/`; hashes in `camera-baseline-build-manifest.json` and run `firmware.json`. Source hashes identify the dirty version precisely.

**No active collector.** Session72562 (`camera-baseline-1`, boot `b028ac611d1a491e66a034fbc13ef2ad`) and session66290 (`live-audio-1`, boot `f75247b219badd835d328177a1638f1c`) are terminal. Preserve original camera-run summary: a host required-check typo (`ina226226_manufacturer`) made it inconclusive. Independent `corrected-check-summary.json` passes the corrected selection and carries capture/integrity errors forward. Do not rewrite the original summary/manifest.

**Verified isolated results:** `live-audio-1` and `camera-baseline-1` each pass live audio, ingress, synthetic speech/FFT and 6,000-block / 60-second audio assessments. Camera run additionally passes 1,800-frame native 1280×720 RGB565 acquisition at ~30 fps, with no observed buffer starvation; driver rejected 640×480. Its final image is hash-verified and visually readable. Camera `-resource-assessment.json` adds strict CPU/stack/SRAM verification to the original `-assessment.json`. See .02 Observed for concise numbers. These are sequential baselines, not combined-load evidence.

**Next independent work:** implement/test a checked BMI270 path and a 100 Hz / 60-second baseline, then UI30fps, JPEG and combined staging. Vendor `accel_gyro_bmi270_init` returns ESP_OK even when Bosch init fails; enable/read helpers discard status. Capture the initialized private Bosch device through a tested `bmi270_init` wrapper, preserve the same BSP bus owner, and use checked Bosch configuration/read APIs. Actual motion was observed earlier, but the helper status is insufficient for sustained validity. Read `sens_time`/status definitions before freshness claims; 200 Hz hardware read at 100 Hz needs an explicit latest-sample policy. Keep tests first and instrument every failure.

The user is **away** and asks us to make autonomous G1 progress. No physical input is pending. Maintain [TODO.md](TODO.md) for operator follow-ups. Wi-Fi credentials are RAM-only and the current boot is not associated; SD remains absent. Prepare mock service and host backpressure work while these wait. Do not count isolated baselines as combined runs, software resets as cold starts, or mock timings as live-provider results.
