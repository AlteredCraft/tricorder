...
`headphones-1` confirms recognizable raw slots 0/2 in both headphone earpieces with better level than the speaker; slots 1/3 are static only. Its unexpected USB-reset attachment failure is preserved; independent strict actual-boot assessment passes with no capture errors. Credentials remain RAM-only. Current firmware includes volume/symbol hints and **Test 10 restarts**; initial boot and radio query pass in `software-restarts-1`. Capture handle 40999 is live; verify it before any USB action. The operator has been asked to tap the button once. Host suite has 52 passing tests. After completion, finalize capture and run `python3 -m tools.restart_evidence .local/runs/20260920-baseline/software-restarts-1`; cold starts are still separate.

Finish .01: physical microphone mapping and louder playback, SD checks when a card is installed, ten cold starts and ten software resets. The deterministic `.03` desktop reference is `tools/audio_reference.py`; four fixtures and direct-DFT outputs are in `.local/runs/20260920-audio-reference-2/`, with 44 host tests passing. `firmware/main/spectrum.cpp` is a fixed-workspace candidate checked against that reference and compiled for P4, but not yet run on the device. Device FFT, speech separation and all hardware/load acceptance still await their prerequisites. Execute .02–.05 against their stated workloads and counts. Do not substitute USB resets for cold starts or declare calibration from raw readings. Ask the user when physical interaction is needed; they are available to operate the device.
