...
**Pending operator question:** one recording near the USB/headphone connector edge, farther microphone covered, then report the visible take ID. Log already contains one accepted button event and complete `audio-1`; await explicit displayed-ID confirmation before assigning its physical position. Only ask the next take after matching the prior one. In earlier `speech-replay-1`, user reported three takes per position (explicitly confirmed three fresh near-connector countdown/RECORDING cycles), but only two operator captures exist, one boot and no sequence gaps. Preserve the mismatch and do not infer physical mapping. Current audio-1 slot-0/2 RMS ratio is -2.76 dB, insufficient alone for mapping.
